...
class Biowulf2(SlurmCluster):
    ...
    def do_submit(self, shell, job, **kwargs):
        ...
        partition = kwargs.get('partition') if kwargs.get(
            'partition') else job.partition
        jobname = kwargs.get('jobname') if kwargs.get('jobname') else job.name
        nodes = kwargs.get('nodes') if kwargs.get('nodes') else job.nodes
        ntasks = kwargs.get("ntasks_per_node") if kwargs.get(
            "ntasks_per_node") else 16
        duration = kwargs.get('duration') if kwargs.get(
            'duration') else '24:00:00'
        ...
        cmd = "cd {}; {} --partition={} --job-name={}  --nodes={} {} --ntasks-per-node={} --time={} {}/submit.sh".format(
            ...
            nodes,
            "--exclusive",
            # if kwargs.get('ntasks_per_node', 16) == 16 else "",
            ntasks,
            duration,
            job.remote_workdir
        )
